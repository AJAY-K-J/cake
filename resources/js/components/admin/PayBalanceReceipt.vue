					...
					<li>
						<form v-on:submit.prevent="get_paybalancereceipts(search)" >  
							<div class="input-group">
								<input type="search" placeholder="sale NO" v-model="search" id="search" class="form-control" style="width:300px;">
				...
				<table class="custom-table table table-hover">
					<thead>
						<tr>
							...
							<th style="width: 60%;">Reciept Details</th>
						...
						<tr v-for="(sale,index) in sales" >
							...
							<td class="newspaper"> 
								...
								<table class='table borderless'>
									<tbody>
										<tr>
											...
											<td align="left">
												<b>{{ getformatteddate(sale.sale_date) }}</b>
											</td>
										</tr>  
										<tr>
											<td>
										...
										<tr>
											...
											<td align="left">
												<b>Rs.{{ parseFloat(sale.total_net_amount)-parseFloat(sale.received_amount) }}</b>
											</td>
										</tr>  
									</tbody>
								</table>
								...
								<td >
									...
									<a class="btn btn-success btn-sm m-0" :href="'/rec_payments/'+sale.id" target="_blank"  style="background:#3e2723">
...
export default {
	...
	methods : {
		...
		get_paybalancereceipts() {
			...
			if(this.company_id)
			{
				var url = '/get_paybalance_receipt?company_id='+this.company_id;  

			}
			else
			{
				var url = '/get_paybalance_receipt?search='+this.search;
