<template>
    <div>
        <form v-on:submit.prevent="save" class="jobCardForm">
            <div class="row">
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" placeholder="Alternate Mobile No" v-model="jobcard.mobile2"  maxlength="10"  autocomplete="on" >
                        <small v-if="errors.mobile2" class="text-danger">{{ errors.mobile2[0] }}</small>
                    </div>
                </div>
                <div class="col-md-4" v-if="jobadvisors[0].company_id!=10">
                    <div class="form-group">
                        <label>Brand<span class="text-danger">*</span></label>

                       <!--  <select class="form-control" v-model="jobcard.make" required @change="get_models_by_make()" ref="make">
                           <option value="">Select Brand</option>
                           <option v-for="make in vehiclemakes" :value="make.name" :data-id="make.id">{{ make.name }}</option>
                       </select> -->
                       <select class="form-control" v-model="jobcard.make" required>
                        <option value="">Select Brand</option>
                        <option v-for="make in vehiclemakes" :value="make.name" >{{ make.name }}</option>
                    </select>
            ...
            <div class="col-md-4" v-else>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" placeholder="Enter Model " v-model="jobcard.model" required  autocomplete="on" style="text-transform: uppercase;">
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        <label>IMEI No/Serial No</label>
                        <input type="text"  class="form-control" placeholder="IMEI No/Serial No" v-model="jobcard.serial_no" maxlength="25"style="text-transform: uppercase;">
            ...
            <div class="col-12">
             ...
             <div class="col-12">
                <div class="form-group">
                    ...
                    <textarea  class="form-control"  v-model="jobcard.technician_voice" maxlength="250"  style="text-transform: uppercase;"> Enter Technician Voice</textarea>
                    <small v-if="errors.technician_voice">{{ errors.technician_voice[0] }}</small>
                </div>
            </div>
            <div class="col-md-3">

                <div class="form-group">
                    <input type="checkbox" id="jobcard.with_warranty" v-model="jobcard.with_warranty">
                    <label for="checkbox"> With Warranty</label>
                </div>
            </div>
            <div class="col-md-3">
            ...
            <div class="col-md-12">
                <h4> <button @click="add_more_issues" class="btn btn-info float-right" type="button">Add Issues</button></h4>
                <div class="table-responsive">
                    <table class="table table-bordered" style="background-color: #f5f8fa;">
                        <thead>
                            <tr>
                                <th style="min-width:10px">S/N</th>
                                <th style="min-width:100px">Select Category</th>
                                <th style="min-width:400px">Select Remarks</th>
                                <th  style="min-width:10px">X</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="(customercategory, index) in jobcard.customercategories">
                                <td>{{ index+1 }}</td>
                                <td>

                                    <select class="form-control" v-model="jobcard.customercategories[index].category_id" required @change="get_catremarks_by_category($event.target.value,index)">
                                        <option value="">Select Category</option>
                                        <option v-for="category in categories" :value="category.id" :data-id="category.id">{{ category.name }}
                                        </option>
                                    </select>
                                </td>      
                                <td>
                                    <select class="form-control" v-model="jobcard.customercategories[index].catremark_id" required>
                                        <option value="">Select Remark</option>
                                        <option v-for="catremark in jobcard.customercategories[index].catremarks" :value="catremark.id">{{ catremark.name }}</option>
                                    </select>
                                    <small v-if="errors.catremark" class="text-danger">{{ errors.catremark[0] }}</small>
                                </td>

                                <td>
                                    <button @click="deleteRow_issues(index)" class="btn btn-danger btn-sm" type="button" style="color: white" > X </button>
                                </td>
                            </tr>
                        </tbody>
                    </table>
            ...
            <div class="col-md-3">
                ...
                <div class="form-group">
                    <input type="checkbox" id="jobcard.battery" v-model="jobcard.battery">
                    <label for="checkbox"> Battery</label>
                </div>
            </div>
            <div class="col-md-3">

                <div class="form-group">
                    <input type="checkbox" id="jobcard.charger" v-model="jobcard.charger">
                    <label for="checkbox"> Charger</label>
                </div>
            </div>
            <div class="col-md-3">

                <div class="form-group">
                    <input type="checkbox" id="jobcard.bag" v-model="jobcard.bag">
                    <label for="checkbox"> Bag/Pouch</label>
                </div>
            </div>
            <div class="col-md-3" v-if="jobadvisors[0].company_id!=8">

                <div class="form-group">
                    <input type="checkbox" id="jobcard.sim" v-model="jobcard.sim">
            ...
            <div class="col-md-3" v-else>
                ...
                <div class="form-group">
                    <input type="checkbox" id="jobcard.dvd_drive" v-model="jobcard.dvd_drive">
                    <label for="checkbox"> CD/DVD Drive</label>
                </div>
            </div>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered" style="background-color: #f5f8fa;">
                        <thead>
                            <tr>
                                <th style="min-width:10px">S/N</th>
                                <th style="min-width:400px">Enter Product/Services</th>
                            ...
                            <tr v-for="(item, index) in jobcard.items">
                                ...
                                <td>
                                    <input class="form-control"
                                      ...
                                      @keyup="set(index)"
                                      @blur="exit(index)"
                                      v-model="jobcard.items[index].service_string"
                                ...
                                <td>
                                   <input type="number" v-model="jobcard.items[index].qty" min="0" required 
                                   class="form-control" placeholder="Quantity"
                                   @change="amount_cal($event.target.value)">
                               </td>
                                <td>
                                    <input type="number" v-model="jobcard.items[index].rate" min="0" required 
    ...
    <div class="col-12">
        <div  class="form-group">
            ...
            <button v-if="company_id || jobcard.reg_status!=2" class="btn btn-primary float-right" type="submit" :disabled="loading">Save</button>
            ...
            <button class="btn btn-danger float-right mr-4" type="button" @click="add_list(jobcard.id)" v-if="type=='edit' && jobcard.reg_status>=1 && !company_id && jobcard.renter_status==0 ">Add to Pending List</button>
             ...
             <button class="btn btn-danger float-right mr-4" type="button" @click="add_adminlist(jobcard.id)" v-if="type=='edit' && jobcard.reg_status==2 && company_id && jobcard.renter_status==0 ">Add to  Pending List by Admin</button>
...
export default {
    props: ['services','type','expired','vehiclemakes','technicians','jobadvisors','company_id','categories','products'],
    created() {
    },
    data () {
        return {
    ...
    methods : {
        mark_jobcarddelivered(id,comp_id) {
          swal({
              title: "Mark Delivery?",
      ...
      clear_data() {
        ...
        this.jobcard.mobile2 = '';
        this.jobcard.technician_voice = '';
        this.jobcard.dealer = '';
    ...
    deleteRow(index) {
        this.jobcard.total_pay=parseInt(this.jobcard.total_pay)-[parseInt(this.jobcard.items[index].qty)*parseInt(this.jobcard.items[index].rate)];
    ...
    amount_cal(id)
    {
        ...
        for(var i=0;i<this.jobcard.items.length;i++)
        {
                ...
                total=total + parseInt(this.jobcard.items[i].rate)*parseInt(this.jobcard.items[i].qty);
                total_spare=total_spare+parseInt(this.jobcard.items[i].rate)*parseInt(this.jobcard.items[i].qty);
    ...
    set(index){
         this.searchFilter =  this.jobcard.items[index].service_string;
    },
    selectOption(option) {
        this.selected = option;
         this.shows[this.index].show = false;
        //this.optionsShown = false;
        this.jobcard.items[this.index].service_string = this.selected.name;
    ...
    set_shows(){
        var i;
        for(i=0;i<this.jobcard.items.length;i++){
           this.shows.push({
                show: false,
            });
    ...
    get_rate_by_product(index){
        ...
        var product = this.jobcard.items[index].service_string;
        if(product==''){
             this.jobcard.items[index].service_string='';
        }
        else{
        axios.post('get_rate_by_product',{ product : product}).then(response => {
            if(response.data==''){
            this.jobcard.items[index].rate = '';
            }
            else{
            this.jobcard.items[index].rate = response.data.mrp;
            this.jobcard.items[index].qty = 1;
            this.amount_cal();
            }
    ...
    save() {
        ...
        then(response => {
            ...
            else if(response.data.response=="no stock"){
                var prod =response.data.product;
                var qty =response.data.qty;
                swal(prod+" has only"+qty+" in Stock", "", "error");
            }
            else
            {
                if(this.type)
...
mounted() {
    if(this.type=='edit') {
        bus.$on('edit-jobcard', (jobcard) => {
            ...
            this.jobcard.mobile2 = jobcard.mobile2;
            ...
            this.jobcard.customercategories = [];
            jobcard.customercategories.forEach((customercategory,index)=>{
                this.jobcard.customercategories.push({
                    category_id: customercategory.category_id,
                    catremark_id: customercategory.catremark_id,
                    catremarks: []
                });
                this.get_catremarks_by_category(customercategory.category_id, index);
            })

            this.$nextTick( function() {
                this.get_models_by_make();
                // this.get_catremarks_by_category();
                this.amount_cal();
            })
