...
export default {
	props: ['purchase' ,'payment_details', 'payment_types', 'type'],
	created() {
	...
	data () {
		return {
			payment: {
				...
				jobcard_no :this.purchase.jobcard_no,
				receipt_no :this.purchase.receipt_no,
				payment_type: '',
				amount: '',
				remarks: '',
				id: '',
	...
	methods : {
		save() {
			this.loading = true;
			axios.post('/purchase_payments/add' ,this.payment).
			then(response => {
			  this.loading = false;
